...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isEmailValid || !isPasswordValid || !form.email || !form.password) return;
    
    setLoading(true);
    setTimeout(() => {
      const result = login(form.email, form.password);
      if (result.success) {
        show(`Welcome back, ${result.user.name.split(' ')[0]}!`, 'success');
        navigate(result.user.role === 'admin' ? '/admin/dashboard' : '/');
      } else {
        show(result.error, 'error');
      }
      setLoading(false);
    }, 1000); // 1s simulated loading
  };
        ...
        <div className="max-w-md mx-auto w-full animate-fadeInUp opacity-0 relative z-10" style={{ animationFillMode: 'forwards' }}>
          ...
          <ul className="space-y-6 mt-12">
            <li className="flex items-center gap-4 text-gray-300">
              ...
              <span className="font-medium">Fast, secure checkout process</span>
            </li>
            <li className="flex items-center gap-4 text-gray-300">
              <div className="w-8 h-8 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0">
                <HiCheck className="text-amber-400 w-5 h-5" />
              </div>
              <span className="font-medium">Track your orders in real-time</span>
            </li>
            <li className="flex items-center gap-4 text-gray-300">
              <div className="w-8 h-8 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0">
                <HiCheck className="text-amber-400 w-5 h-5" />
              </div>
              <span className="font-medium">Exclusive member-only deals</span>
            </li>
          </ul>
      ...
      <div className="w-full lg:w-[55%] flex flex-col justify-center px-8 sm:px-16 xl:px-24 py-12 relative bg-white">
        ...
        <div className="max-w-md w-full mx-auto animate-fadeInUp opacity-0 animate-delay-120" style={{ animationFillMode: 'forwards' }}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="relative pt-2 pb-2">
              ...
              <button
                ...
                className="absolute right-0 top-2.5 text-gray-400 hover:text-primary transition-colors focus:outline-none"
              >
                {showPassword ? <HiOutlineEyeOff className="w-5 h-5" /> : <HiOutlineEye className="w-5 h-5" />}
              </button>
              <div className="absolute right-0 -top-6">
                <Link to="/forgot-password" className="text-xs font-semibold text-gray-500 hover:text-amber-500 transition-colors">Forgot?</Link>
              </div>
            </div>

            <button
              type="submit"
              disabled={loading || !form.email || !form.password || !isEmailValid || !isPasswordValid}
          ...
          <p className="text-center text-sm text-gray-500 mt-8">
            ...
            <Link to="/register" className="font-bold text-[#0A1628] hover:text-amber-500 transition-colors">Create one</Link>
          </p>
        </div>
      </div>
    </div>
